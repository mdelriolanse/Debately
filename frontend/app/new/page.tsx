...
export default function NewDebatePage() {
  ...
  const handleCreateTopicWithProposition = async (proposition: string) => {
    ...
    try {
      ...
      if (!user) {
        throw new Error('You must be signed in to create a debate topic')
      }
      ...
      // created_by will be set by backend from authenticated user
      const topicResponse = await createTopic({
        proposition: proposition.trim(),
        created_by: user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split('@')[0] || 'user'
      })

      const topicId = topicResponse.topic_id

      // Get username for arguments
